---
title: '3. Upload data to the storage account'
layout: default
nav_order: 3
parent: 'Exercise 00: Provision and configure resources'
---

# Task 03: Upload data to the storage account


### Key steps



#### 01: Get the access key for the storage account
1. On the Azure home page, in the **Search** field, search for and select `Storage accounts`.

    ![dz2xd7cr.jpg](../../media/dz2xd7cr.jpg)

    {: .warning }
    > Be sure to use **Storage accounts** and not **Storage accounts (classic)**. The instructions and screenshots will not be correct if you use **Storage accounts (classic)**.

1. On the **Storage Accounts** page, select **litwareuc@lab.LabInstance.Id**.

    ![hadzz3th.jpg](../../media/hadzz3th.jpg)

1. In the left pane, select **Security + networking** and then select **Access keys**.

    ![ipy0kikn.jpg](../../media/ipy0kikn.jpg)

1. In the **key1** section, in the **Key** field, select **Show**.

    ![xugcfbd5.jpg](../../media/xugcfbd5.jpg)

1. Copy the key and paste it into the following text field. You will use the key later in the workshop to create a shortcut from the Fabric workspace to the data.

    @lab.TextBox(LitWareDataAccessKey)

---

#### 02: Upload files to the storage account

1. On the desktop, select the shortcut to launch **Azure Storage Explorer**.

1. Select **Sign in with Azure**.

1. In the **Select Azure environment** dialog, select **Azure** and then select **Next**.

    ![w8qjliw5.jpg](../../media/w8qjliw5.jpg)
    ![ckmso6g2.jpg](../../media/ckmso6g2.jpg)

1. Sign in by using the following credentials:

    | Setting | Value |
    |:---------|:---------|
    | Username   | `@lab.CloudPortalCredential(User1).Username`   |
    | Temporary Access Pass (TAP) token   | `@lab.CloudPortalCredential(User1).AccessToken`   |

1. On the **Account Management** pane, select **Open Explorer**.

    ![41szw8yh.jpg](../../media/41szw8yh.jpg)

1. In the **Explorer** pane, expand the folder with the key symbol and then expand **Storage Accounts**. 

    ![tosmdwnx.jpg](../../media/tosmdwnx.jpg)

1. Expand **litwareuc@lab.LabInstance.Id**. 

    ![am1v4uml.jpg](../../media/am1v4uml.jpg)

1. Right-click **Blob Containers** and then select **Create Blob Container**.

    ![7jh7sa1m.jpg](../../media/7jh7sa1m.jpg)

1.  In the text field, enter `litwaredata` and then select **Enter**.

   ![xcxa94s3.jpg](../../media/xcxa94s3.jpg)

1. On the new tab that opens, on the command bar, select **Upload** and then select **Upload Folder**.

    ![xdhoby8r.jpg](../../media/xdhoby8r.jpg)

1. In the **Upload Folder** dialog, in the **Selected folder** field, select the ellipses (**...**).

    ![0zjzzeiq.jpg](../../media/0zjzzeiq.jpg)

1. In the **Choose folder to upload** dialog, go to `C:\Lab Assets\LitwareData\litwaredata`. Select **data** and then select **Select folder**.

    ![5rp8q391.jpg](../../media/5rp8q391.jpg)

1. Select **Upload**.

    ![61420vuc.jpg](../../media/61420vuc.jpg)

1. Wait while the files upload.

1. Repeat Steps 10-14 to upload the **litwaredata** and **Products** folders.

1. In the **litwaredata** pane, verify that the files have uploaded.

    ![q4wmpgeu.jpg](../../media/q4wmpgeu.jpg)

1. Close **Azure Storage Explorer**.